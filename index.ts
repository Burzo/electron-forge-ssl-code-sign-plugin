...
export class ElectronForgeSslCodeSignPlugin extends PluginBase<ConfigTypes> {
    ...
    postMake = async (
        ...
        const squirrelMaker: ForgeConfigMaker | undefined =
        ...
        return makeResults.map((data) => {
            ...
            if (!userName || !password || !credentialId || !signToolPath) {
                throw new Error(
                    `You did not provide all the required config variables to ${
                        ...
                        .map(
                            (key) =>
                                `${key}  -  ${this.config[key as keyof ConfigTypes]}`,
                        )
                        .join("\n")}`,
                );
            }
            ...
            if (releasesPath.endsWith("RELEASES")) {
                ...
                const nupkgFilePath = releasesPath.replace(
                    "RELEASES",
                    nupkgFileName,
                );
                 ...
                 * )
                 */
                const { dir: codeSignToolFolder, name } =
                    path.parse(signToolPath);
